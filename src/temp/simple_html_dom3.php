...
function dump_html_tree3($node, $show_attr = true, $deep = 0)
{
	$node->dump($node);
}

class simple_html_dom_node3
{
...
class simple_html_dom3
{
		...
		$defaultSpanText = DEFAULT_SPAN_TEXT)
	{
		...
		$this->root = new simple_html_dom_node3($this);
		$this->root->tag = 'root';
	...
	protected function parse()
	{
		while (true) {
			...
			$node = new simple_html_dom_node3($this);
			++$this->cursor;
	...
	{
		return @str_get_html3("<$name>$value</$name>")->firstChild();
	}

	function createTextNode($value)
	{
		return @end(str_get_html3($value)->nodes);
	}
